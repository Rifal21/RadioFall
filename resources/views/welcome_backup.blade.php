...
<body class="text-black min-h-screen flex flex-col p-4 md:p-8">
    ...
    <!-- Navbar -->
    <nav
        class="neo-box bg-cyan-300 rounded-xl px-6 py-4 flex flex-col md:flex-row justify-between items-center mb-8 sticky top-4 z-50">
        <div class="flex items-center gap-4 mb-4 md:mb-0 transform hover:-rotate-2 transition-transform">
            <div
                class="w-12 h-12 bg-black text-white rounded-full flex items-center justify-center border-2 border-white shadow-[2px_2px_0_0_rgba(0,0,0,0.5)]">
                <i class="fa-solid fa-radio text-xl animate-pulse"></i>
            </div>
            <h1 class="text-4xl font-extrabold tracking-tight italic uppercase" style="text-shadow: 2px 2px 0px white;">
                Radio<span class="text-pink-500">Fall</span></h1>
        </div>
        <button onclick="openRequestModal()"
            class="neo-btn bg-yellow-400 hover:bg-yellow-300 text-black font-black py-3 px-8 rounded-lg text-lg uppercase tracking-wide flex items-center gap-3">
    ...
    <main class="flex-grow grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <!-- Right Column: Chat -->
        <div class="neo-box bg-white rounded-xl flex flex-col h-[600px] lg:h-auto overflow-hidden relative">
            ...
            <div id="chat-container"
                ...
                @foreach ($messages as $msg)
                    <div class="flex flex-col group">
                        <span
                            class="text-sm font-black text-black mb-1 ml-2 bg-yellow-300 inline-block px-2 border-2 border-black -rotate-1 self-start shadow-[2px_2px_0_0_rgba(0,0,0,1)]">{{ $msg->sender_name }}</span>
                        <div
                            class="bg-white border-3 border-black p-3 rounded-2xl rounded-tl-none shadow-[4px_4px_0_0_rgba(0,0,0,0.1)] group-hover:shadow-[4px_4px_0_0_rgba(0,0,0,1)] transition-all">
            ...
            <div class="p-4 bg-yellow-100 border-t-4 border-black">
                <form id="chat-form" onsubmit="event.preventDefault(); sendMessage();" class="flex flex-col gap-3">
                    <input type="text" id="chat-name" name="sender_name" placeholder="YOUR NAME"
                        class="neo-input w-full p-3 rounded-lg font-bold uppercase text-sm" required>
    ...
    <script>
            ...
            songs.forEach(song => {
                ...
                htmlContent += `
                    <div class="flex justify-between items-center p-3 bg-white border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,0.1)] hover:shadow-[4px_4px_0_0_#fbbf24] hover:-translate-y-1 transition-all rounded-lg group mb-2">
                        ...
                        <button onclick="submitRequest('${safeTitle}', '${safeArtist}')" class="bg-black hover:bg-gray-800 text-white font-bold text-xs px-4 py-2 rounded-md shadow-[2px_2px_0_0_rgba(0,0,0,0.2)]">
        ...
        function submitRequest(songTitle, songArtist) {
            Swal.close();
            ...
            setTimeout(() => {
                Swal.fire({
                    ...
                    customClass: {
                        ...
                        input: 'border-3 border-black rounded-lg shadow-[4px_4px_0_0_rgba(0,0,0,0.1)] focus:shadow-[4px_4px_0_0_#fbbf24] focus:outline-none font-bold uppercase',
                        confirmButton: 'neo-btn',
                        cancelButton: 'neo-btn'
                    },
                    preConfirm: (name) => {
                        if (!name) {
                            Swal.showValidationMessage('HEY! WE NEED A NAME!');
                        }
                        return name;
                    }
                }).then((result) => {
                            ...
                            .then(data => {
                                if (data.success) {
                                    Swal.fire({
                                        icon: 'success',
                                        title: 'BOOM! SENT!',
                                        text: 'Your request is flying to our DJ!',
                                        timer: 3000,
                                        showConfirmButton: false,
                                        background: '#fff',
                                        color: '#000',
        ...
        function sendMessage() {
            ...
            const tempHtml = `
                 <div class="flex flex-col group opacity-50" id="temp-msg">
                    <span class="text-sm font-black text-black mb-1 ml-2 bg-gray-200 inline-block px-2 border-2 border-black -rotate-1 self-start">${name}</span>
                    <div class="bg-white border-3 border-black p-3 rounded-2xl rounded-tl-none shadow-[4px_4px_0_0_rgba(0,0,0,0.1)]">
            ...
            fetch('/chat/messages')
                ...
                .then(data => {
                    ...
                    data.forEach(msg => {
                        ...
                        // Alternate colors/styles could be added here randomly or by id
                        html += `
                        <div class="flex flex-col group animate-fade-in">
                            <span class="text-sm font-black text-black mb-1 ml-2 bg-yellow-300 inline-block px-2 border-2 border-black -rotate-1 self-start shadow-[2px_2px_0_0_rgba(0,0,0,1)]">${msg.sender_name}</span>
                            <div class="bg-white border-3 border-black p-3 rounded-2xl rounded-tl-none shadow-[4px_4px_0_0_rgba(0,0,0,0.1)] group-hover:shadow-[4px_4px_0_0_rgba(0,0,0,1)] transition-all">
                                <p class="text-black font-medium leading-tight">${msg.message}</p>
                            </div>
        ...
        if (audioPlayer) {
            ...
            function fetchRequests() {
                fetch('/request/list')
                    .then(response => response.json())
                    .then(data => {
                        const container = document.querySelector('.mt-8.neo-box .space-y-3'); // Target the container
                        if (!container) return;
                        ...
                        if (data.length === 0) {
                            ...
                            data.forEach(req => {
                                // Calculate time diff roughly or use moment.js if available. 
                                // For simplicity, we just show 'Just now' or static time if we don't want to import moment.
                                // But since we want it dynamic, let's keep it simple or use JS date math.
            ...
            setInterval(fetchRequests, 10000); // Check for requests every 10s
        }
    </script>
</body>
